...
def is_video_path(path, extensions=('mp4', 'avi')):
    files = glob.glob(path)  # Find all matching files
    if not files:
        return False  # No files found

    return any(file.endswith(f'.{ext}') for ext in extensions for file in files)
